...
class PhotoApp(MDApp):
    ...
    def _load_kv_files(self):
        ...
        # Load shared rules first, if any
        if os.path.exists(auth_kv):
            try:
                Builder.load_file(auth_kv)
                Logger.info("KV: loaded rules from auth.kv")
            except Exception as e:
                Logger.warning(f"KV: failed loading auth.kv rules: {e}")

        # Return the first KV that can provide a root widget
        for kv in (app_kv, auth_kv, main_kv):
            if os.path.exists(kv):
                try:
                    root = Builder.load_file(kv)
                    Logger.info(f"KV: root from {os.path.basename(kv)}")
                    return root
                except Exception as e:
                    Logger.warning(f"KV: {os.path.basename(kv)} failed as root: {e}")
    ...
    def change_screen(self, name: str):
        ...
        try:
            ...
            pass
                ...
                # main.py  (inside change_screen, after setting sm.current)
    ...
    def save_profile(self):
        ...
        def _save_user_profile(self):
            """KV/back-compat alias: persist current profile into users/<mobile>/profile.json."""
            mobile = (self.profile_data.get("mobile") or "").strip()
            if not re.fullmatch(r"[0-9]{10}", mobile):
